...
function get_extreme_period(DF, GDF, profKey, typeKey, statKey, demand_col_names, solar_col_names, wind_col_names, inflow_col_names; v=false)
    ...
    if typeKey == "Integral"
        ...
        elseif profKey == "Inflow"
            (stat, group_idx) = get_integral_extreme(GDF, statKey, inflow_col_names)
        else
            println(" -- Error: Profile Key ", profKey, " is invalid. Choose `Load', `PV', `Wind', or `Inflow'.")
        end
    elseif typeKey == "Absolute"
        ...
        elseif profKey == "Inflow"
            (stat, group_idx) = get_absolute_extreme(DF, statKey, inflow_col_names)
        else
            println(" -- Error: Profile Key ", profKey, " is invalid. Choose `Load', `PV', `Wind', or `Inflow'.")
        end
   else
...
function create_reduced_csv(full_path::String, reduced_path::String, M::Vector{Int}, hours_per_period::Int; v=false)
    ...
    file_type =
        ...
        elseif occursin("avail", filename_lower)
            "Availability"
        elseif occursin("evap", filename_lower)
            "Evaporation"
        elseif occursin("inflow", filename_lower)
            "Inflow"
        else
            "Time Series"
        end
