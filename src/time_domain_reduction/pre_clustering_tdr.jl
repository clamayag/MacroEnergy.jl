function pre_clustering_tdr(case_path::String, mysetup::Dict; period_idx::Int = 1, v::Bool=false)
    ...
    ClusterEvap = mysetup["ClusterEvap"]
    ClusterInflow = mysetup["ClusterInflow"]
    ...
    if mysetup["ClusterEvap"] == 1
        evap_base_name = mysetup["EvapFileName"]
        if GLOBAL_NUM_PERIODS[] == 1
            evap_file_name = evap_base_name * ".csv"
        else
            evap_file_name = evap_base_name * "_" * string(period_idx) * ".csv"
        end
        evap_path = joinpath(system_path, evap_file_name)
        evap = DataFrame(CSV.File(evap_path))
        Nhours_full = (nrow(evap) ÷ TimestepsPerRepPeriod) * TimestepsPerRepPeriod
        evap = evap[1:Nhours_full, :]
        if v println("Loaded evaporation $(size(evap)) from $evap_path") end
    end

    if mysetup["ClusterInflow"] == 1
        inflow_base_name = mysetup["InflowFileName"]
        if GLOBAL_NUM_PERIODS[] == 1
            inflow_file_name = inflow_base_name * ".csv"
        else
            inflow_file_name = inflow_base_name * "_" * string(period_idx) * ".csv"
        end
        inflow_path = joinpath(system_path, inflow_file_name)
        inflow = DataFrame(CSV.File(inflow_path))
        Nhours_full = (nrow(inflow) ÷ TimestepsPerRepPeriod) * TimestepsPerRepPeriod
        inflow = inflow[1:Nhours_full, :]
        if v println("Loaded inflow $(size(inflow)) from $inflow_path") end
    end
    ...
    drop_time_columns!(evap)
    drop_time_columns!(inflow)
    ...
    if ClusterEvap == 1 push!(dfs_to_combine, evap) end
    if ClusterInflow == 1 push!(dfs_to_combine, inflow) end 
    if ClusterSubperiodResults == 1 push!(dfs_to_combine, subperiod_results) end

    if isempty(dfs_to_combine)
        error("No active time series selected for clustering. Enable at least one of ClusterAvailability, ClusterDemand, ClusterFuelPrices, ClusterEvap, ClusterInflow, or ClusterSubperiodResults in TDR_settings.json.")
    end
    ...
    demand_col_names = [string(c) for c in names(InputData)
        ...
        inflow_col_names = [string(c) for c in names(InputData)
            if occursin("inflow", lowercase(string(c))) && !occursin("edge", lowercase(string(c)))]
                        ...
                        if ExtPeriodSelections[profKey][geoKey][typeKey][statKey] == 1
                            if geoKey == "System"
                                (stat, group_idx) = get_extreme_period(
                                    ...
                                    demand_col_names, solar_col_names, wind_col_names, inflow_col_names; v=v
                                )                   
