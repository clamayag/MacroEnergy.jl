...
######### Derived time utilities #########
function timestep_seconds(y::Union{AbstractVertex,AbstractEdge}, t::Int)
    w = current_subperiod(y, t)
    ω = haskey(y.timedata.subperiod_weights, w) ?
            subperiod_weight(y, w) : 1.0
    return hours_per_timestep(y) * ω * 3600
end
######### Derived time utilities #########
